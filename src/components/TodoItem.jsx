...
const TodoItem = ({ data, deleteTodo, setEditMode, markCompleteTodo }) => {
    const date = new Date(data.id);

    const [completed, setCompleted] = useState(data.completed)

    useEffect(()=>{
        markCompleteTodo(data.id, completed)
    }, [completed])
        ...
        <div className="p-4">
            <div className="flex justify-between">
                <div className="flex gap-3 items-center">
                    <input type="checkbox" checked={completed} onChange={()=> setCompleted(!completed)} className="w-4 h-4 accent-violet-500 cursor-pointer" />
                ...
                <div className="flex gap-4 items-center">
                    <MdEdit color="white" size={20} className="cursor-pointer" onClick={()=> setEditMode(data.id) } />
